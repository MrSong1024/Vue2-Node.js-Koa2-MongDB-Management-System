import { initData, download } from '@http/data'
...
function CRUD(options) {
    ...
    const methods = {
        ...
        disableSuccessNotify() {
            crud.notify(crud.msg.disable, CRUD.NOTIFICATION_TYPE.SUCCESS)
        },
        enableSuccessNotify() {
            crud.notify(crud.msg.enable, CRUD.NOTIFICATION_TYPE.SUCCESS)
        },
        ...
        toAdd() {
            ...
            if (crud.isHaveAttr) {
                crud.attribute = {}
                let form = crud.attributeOptions
                const crudFrom = crud.attribute
                form.forEach(val => {
                    Vue.set(crudFrom, val.attrKey, "")
                })
            }
        },
        /**
        ...
        toEdit(data) {
            ...
            crud.getDataStatus(data._id).edit = CRUD.STATUS.PREPARED
            callVmHook(crud, CRUD.HOOK.afterToEdit, crud.form)
            callVmHook(crud, CRUD.HOOK.afterToCU, crud.form)

            // 含有拓展属性
            if (crud.isHaveAttr && crud.form.extendAttrs) {
                const extendAttrs = JSON.parse(crud.form.extendAttrs);
                extendAttrs.forEach((val) => {
                    crud.attributeOptions.forEach((v) => {
                        if (v.attrKey === val.name) {
                            Vue.set(crud.attribute, val.name, val.value);
                        }
                    });
                });
            }
         ...
         */
        toDelete(data) {
            crud.getDataStatus(data._id).delete = CRUD.STATUS.PREPARED
        },
        /**
         * 取消删除
         * @param {*} data 数据项
        ...
        cancelDelete(data) {
            ...
            crud.getDataStatus(data._id).delete = CRUD.STATUS.NORMAL
        ...
        cancelCU() {
            ...
            if (editStatus === CRUD.STATUS.PREPARED) {
                ...
                const id = crud.form._id
                crud.getDataStatus(id).edit = CRUD.STATUS.NORMAL
            }
            crud.resetForm()
            if (addStatus === CRUD.STATUS.PREPARED) {
        ...
        doAdd() {
            ...
            crud.btnLoading = true
            crud.crudMethod
                .add(crud.form)
                .then(() => {
                    crud.btnLoading = false
                    crud.status.add = CRUD.STATUS.NORMAL
                    crud.resetForm()
        ...
        doDelete(data) {
            ...
            if (data instanceof Array) {
                ...
                ids.push(data._id)
                dataStatus = crud.getDataStatus(data._id)
            }
        ...
        toDisable(data) {
            ...
            const ids = crud.getDataStatusArr(data)
            crud.crudMethod
                .disable({ ids })
                .then((res) => {
                    crud.disableSuccessNotify()
                    crud.resetForm()
                    callVmHook(crud, CRUD.HOOK.afterSubmit)
                    crud.refresh()
                })
        ...
        toMofify(ids) {
            crud.crudMethod
                .enable(ids)
                .then((res) => {
                    crud.notify('启用类型成功', 'success')
                    this.crud.toQuery()
                })
                .catch(() => {
                    crud.notify('启用类型失败', 'error')
                })
        },
            ...
            if (
                ...
                if (row.children) {
                    row.children.forEach((val) => {
                        crud
                            .findVM('presenter')
                            .$refs['table'].toggleRowSelection(val, true)
                        selection.push(val)
                        if (val.children) {
                            crud.selectChange(selection, val)
                        }
        ...
        toggleRowSelection(selection, data) {
            if (data.children) {
                data.children.forEach((val) => {
                    crud
                        .findVM('presenter')
                        .$refs['table'].toggleRowSelection(val, false)
                    if (val.children) {
...
function presenter(crud) {
    ...
    return {
        ...
        mounted() {
            const columns = {}
            this.$refs.table && this.$refs.table.columns.forEach((e) => {
