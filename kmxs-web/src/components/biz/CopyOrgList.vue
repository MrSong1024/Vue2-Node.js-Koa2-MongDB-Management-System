<template>
  <div id="userList">
    ...
    <div class="head-container crud-opts ui-flex">
      <div class="crud-opts-left ui-flex ui-flex-1">
        <!-- 搜索 -->
        <el-input
      ...
      <div class="crud-opts-right ui-flex">
        ...
        <el-dropdown trigger="click">
          ...
          <el-dropdown-menu slot="dropdown" style="width: 150px">
            ...
            <el-dropdown-item>
              <el-button class="filter-item" type="text" size="mini"
                >移动</el-button
              >
            </el-dropdown-item>
            <el-dropdown-item>
      ...
      >
        ...
        <el-form-item label="直属上级" prop="leaderName">
          ...
          <el-autocomplete
            ...
            @select="handleSelectUser"
          />
          <el-button type="text" size="mini" class="O3-btn" @click="toUser()"
            >选择上级</el-button
          >
        </el-form-item>
    ...
    >
      ...
      <el-table-column label="操作" width="130" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button size="mini" type="text" @click="crud.toEdit(scope.row)"
            >编辑</el-button
          >
        </template>
      </el-table-column>
    ...
    >
      <user-selector
        ...
        :show-advanced-filter="userListAdvancedFilterDialog"
...
export default {
  ...
  // 数据字典
  dicts: ["user_status", "approve_status"],
  props: {
    selectType: {
      type: String,
      default: "radio",
  ...
  data() {
    return {
      ...
      userListAdvancedFilterDialog: false,
    };
  },
  computed: {
    ...mapGetters(["user"]),
  ...
  methods: {
    ...
    toMofifyApproval(ids, bizAction) {
      if (!ids || ids.length <= 0) {
        ...
        if (Array.from(new Set(typeList)).length > 1) {
          ...
          // 项目合作方及编外用户延期时，
          // 如果所选用户的所属项目（合作方用户）或所属部门（编外用户）为空，
          // 则提示”用户所属项目为空“或”用户所属部门为空“
          // 编外E2 项目合作方O2
          // if (selection[i].type === 'O2' && selection[i].itSupplier === null) {
          //   return false
          // }
          if (E2TypeList) {
            ...
            console.log("编外用户所属部门：>>", deptIdList);
            if (!deptIdList.every((item) => item !== null)) {
              this.$message.error({
                message: "用户所属部门为空！",
                duration: 2000,
              });
          ...
          if (O2TypeList) {
            ...
            console.log("项目合作方用户：>>", projectIdList);
            if (!projectIdList.every((item) => item !== null)) {
              this.$message.error({
                message: "用户所属项目为空！",
                duration: 2000,
              });
          ...
          if (O3TypeList) {
            ...
            const attributesList = ids.map((item) => {
            ...
            if (Array.from(new Set(projectIdList)).length > 1) {
              ...
              Array.from(new Set(projectIdList)).length === 1 &&
              Array.from(new Set(attributesList)).length > 1
            ) {
              this.$message.error({
                message: "请选择同一属性的用户(IT或非IT)！",
                duration: 2000,
              });
              return false;
            } else {
    ...
    handleUserSelChange(params) {
      const data = params[0];
      this.form.leaderName = data.nickName;
      this.form.leaderEmployNum = data.employNum;
